...
! CollapsingSpheres potential


subroutine ll_init_model(N_params, params)
use example_CollapsingSpheres_params_mod
implicit none
   integer :: N_params
   ...
   if (N_params==1) then
       ...
       write(*,*) "No CollapsingSphere parameters found, the default will be used:"
       sigma(1)   =  1.0d0
       sigma1(1)  =  1.35d0
       kk(1)      = 10.0d0
       cutoff(1)  =  sigma1(1) + 2.0d0 ! BEWARE of this cutoff and check it works with your choice of parameters.
       cutoff_sq  = cutoff*cutoff
       e_shift(1) = (sigma(1)/cutoff(1))**14 + 0.5d0*(1-tanh(kk(1)*(cutoff(1)-sigma1(1)))) 

   else

       write(*,*) "The following CollapsingSphere parameters will be used:"
       sigma(1) =  1.0d0
       sigma1(1)  =  params(1) !1.35d0
       kk(1)      =  params(2) !10.0d0
       cutoff(1)  = sigma1(1) + 2.0d0 ! BEWARE of this cutoff and check it works with your choice of parameters.
       cutoff_sq = cutoff*cutoff
       e_shift(1) = (sigma(1)/cutoff(1))**14 + 0.5d0*(1-tanh(kk(1)*(cutoff(1)-sigma1(1)))) 

   endif
   write(*,*) "Core",sigma(1),"widths of the step, sigma1:",sigma1(1)
   write(*,*) "smoothness of step",kk(1),"cutoff",cutoff(1)
   write(*,*) "energy shift:", e_shift(1) 
   ...
   do j=i, N
	 ...
	 if (dr_mag_sq < cutoff_sq(1)) then
	    dr_mag = sqrt(dr_mag_sq)
            ...
            E_term = (sigma(1)/dr_mag)**14 + 0.5d0*(1-tanh(kk(1)*(dr_mag-sigma1(1)))) - e_shift(1)
...
implicit none
   ...
   write(*,*) "Forces are not available for the CollapsingSpheres potential. Use MC. Stop."
   stop
