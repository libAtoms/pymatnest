   ...
   do j=i, N
	 ...
	 if (dr_mag_sq < cutoff_sq(1)) then
	    dr_mag = sqrt(dr_mag_sq)
	       ...
	       E_term = w_a(1) - w_a(1)*((dr_mag-d_a(1))/(cutoff(1)-d_a(1)))	    
	    elseif (dr_mag >= sigma(1) ) then
	       E_term = w_r(1) - (w_r(1) - w_a(1))*(dr_mag-sigma(1))/(d_a(1)-sigma(1))	   
	    else
	       E_term = huge(1.0d0)     
	    endif	    
   ...
   atoms_c: do j=1,N
	 ...
	 if (dr_mag_sq < cutoff_sq(1)) then
	    ...
	    if (dr_mag <= sigma(1)) then
	       ...
	       exit atoms_c
            ...
            if (dr_mag >= d_a(1)) then
	       dE = dE - ( w_a(1) - w_a(1)*((dr_mag-d_a(1))/(cutoff(1)-d_a(1))) )
	    else
	       dE = dE - ( w_r(1) - (w_r(1) - w_a(1))*(dr_mag-sigma(1))/(d_a(1)-sigma(1)) )   
	    endif	    
	 ...
	 if (drp_mag_sq < cutoff_sq(1)) then
	    ...
	    if (drp_mag <= sigma(1)) then
	       ...
	       dE = dE + ( w_a(1) - w_a(1)*((drp_mag-d_a(1))/(cutoff(1)-d_a(1))) )
	    else
	       dE = dE + ( w_r(1) - (w_r(1) - w_a(1))*(drp_mag-sigma(1))/(d_a(1)-sigma(1)) )   
	    endif	    
