function fortran_MC(N, pos, cell, n_steps, step_size, Emax) result(n_accept)
   implicit none
   integer :: N
   double precision :: pos(3,N), cell(3,3)
   integer :: n_steps
   double precision :: step_size, Emax
   ! result
   integer :: n_accept
   ...
   n_accept = 0
   E = ll_eval_energy(pos, cell)
   do i=1, n_steps
      call random_number(d_r)
      d_i = floor(d_r*N)+1
      call random_number(d_pos)
      d_pos = step_size*(d_pos-0.5)
